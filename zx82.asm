...
; Changes w/r the original ZX Spectrum 16/48 ROM
;
...
; NMI fixed, performs warm restart, unless disabled
; STR_AND_NO minor speedup (PUSH DE,POP DE replaced by INC DE, INC DE)
; SWAP_BYTE sped up, saves one byte
...
; It fixes most frustrating bugs, improves performance and lays the groundwork
; for extensibility, but, importantly, does not change functionality and
; retains as much compatibility with Sinclair and 3rd party hardware and
; software as possible. Its primary specification is Steve Vickers' manual.
; The project is dedicated to Steve Vickers.

; Copyright: my own modifications are covered by the MIT license, but this
; particular file is definitely NOT. In fact, I do not know the exact status
; of its copyright, as it relies on many people's work. The bulk of it is
; under Amstrad's copyright, as they own the rights to the original ROM.
...
; in the production ZX Spectrum 16k ROM should have a lablel consisting of a
; capital L followed by the hexadecimal address, so that in a listing it can
; be checked that their address has not been shifted by accident. Entry points
; that are functionally identical to old entry points, but have changed address,
; should have the same labels as those in the Logan/O'Hara book.
...
;;; Reset stream by calling its output service routine with A=0 and CF=0
RESET_STREAM:
	CALL	L1601		; Find correspondinc channel information
	XOR	A		; Clear CF and register A to signal reset
	JP      L15F2           ; Jump forward to continue at old PRINT-A-2.
...
;;;     JR      NZ,L0070        ; skip to NO-RESET if NOT ZERO
        JR      Z,L0070         ; skip to NO-RESET if ZERO
...
L0609:  ;SUB     (1 + L1ADF) % $100 ; subtract the known offset.
        SUB	$E0             ; ( is SUB $E0 in standard ROM )
...
;  type is a program or an array.
        ...
        JP      Z,MECTRL	; jump forward to ME-CONTRL if so.
...
;;;L0A5F:
PO_COMMA:
;;;	CALL    L0B03           ; routine PO-FETCH - seems unnecessary.
...
;;; BUGFIX: Rearrangement
        JR      POCHANGE       ; forward to PO-CHANGE >>

;; PO-TV-2
;;; BUGFIX: Check if channel is being reset
;;;L0A6D:
PO_TV_2:JR	NC,PO_CONT
	LD      DE,PO_CONT	; address: PO-CONT will be next output routine
...
;;;L0AAC:
PO_AT_ERR:
	JP      C,L1E9F         ; to REPORT-B if higher than 22 decimal
...
;;;L0ABF:
PO_AT_SET:
	JP      L0DD9           ; print position is valid so exit via CL-SET
...
;; PO-FILL
;;;L0AC3:
PO_FILL:CALL    L0B03           ; routine PO-FETCH, HL-addr, BC=line/column.
...
;; PO-ST-E
;;;L0AF0:
PO_ST_E:LD      (S_POSNL),BC      ; Update SPOSNL line/column lower screen
...
;;;L0AFC:
PO_ST_PR:
	LD      (IY+$45),C      ; Update P_POSN column position printer
...
;   Overwrite registers with values for lower screen.
        ...
        LD      BC,(S_POSNL)      ; Fetch line/column from SPOSNL
...
X0BE4:	LD      DE,(ATTR_T)      ; make D hold ATTR_T, E hold MASK-T
...
; ---
        LD      B,$17           ; set count to whole display ??
...
;;; BUGFIX: Reset K channel coordinates befoe setting address
	LD      BC,$1721        ; line 23 for lower screen
;;; BUGFIX: Use new PRINT_OUT and set channel after coordinates
RESET_PS_1:
	LD	DE,PRINT_OUT
	LD      HL,(CURCHL)      ; fetch CURCHL to HL to address current channel
...
;
;   NMI discriminator
	LD	A,(ERR_NR)	; Test ERR_NR -- zero after NEW and RESET
	OR	A
	JP	NZ,L1303
; ---
        LD      HL,CHINFO        ; The address of the channels - initially
...
;;;        LD      (STKEND),HL      ; set STKEND to the end of the empty stack.
        ...
        LD      C,$0E           ; copy the 14 bytes of initial 7 streams data
...
; The NMI routine, 8 bytes
NMIVEC:	LD	A,$14		; BREAK into program
	LD	(ERR_NR),A	; ERR_NR is zeroed by RESET and NEW.
...
;; init-chan
L15AF:  DEFW    PRINT_OUT       ; PRINT-OUT
...
;;;        ADD     A,E             ; '0' to '9' and continue into the main output
                                   ; routine.
L15EF:	ADD	A,"0"	; Legible error codes
...
PRINT_A_2:
	SCF		; Set Carry for output
;; Old PRINT-A-2, calls the output/ioctl service routine
...
;;;        CALL    L2D3B           ; routine INT-TO-FP will read digits till
                                ; a non-digit found.
	CALL	DEC2FP		; Line numbers must be decimal
;---
...
;; LINE-NEW
;;; BUGFIX: attempt binary search for line number
L1B9E:  CALL	LINE_ADDR	; routine LINE-ADDR gets address of line
;;;	CALL    L196E           ; routine LINE-ADDR gets address of line
...
L1CA5:  ;SUB     L1AEB-$D8 % 256 ; convert $EB to $D8 ('INK') etc.
        SUB	$13             ; ( is SUB $13 in standard ROM )
...
;;; BUGFIX: save one byte and 1 cycle by moving NEXT without FOR
	JR	NZ,REPORT2
...
;;; BUGFIX: local variables behave a bit differently
	JP	Z,REPORT1
;;;        JR      Z,L1DD8         ; forward to REPORT-1 if not
                                ; 'NEXT without FOR'

X1DB9:	INC     HL              ; step past variable name
...
;; REPORT-1
;;; BUGFIX: make this ERROR2 and move ERROR1
REPORT2:RST	$08
	DEFB	$01		; Error Report: Variable not found
...
L2831:  AND     A               ; test A ( will be zero if string '$' - '$' )
        ...
        JR      Z,SFRBR2         ; forward to SF-R-BR-2 if no arguments.
;; SF-ARG-LP
...
;;;        DEC     HL              ; step back one
        ...
        JP      Z,SAVARGS         ; forward to SAVARGS if so.
...
;; SF-R-BR-2
SFRBR2:	PUSH    HL              ; save location of ')' in DEF FN
...
DEC2FP:	LD	C,$0A
	CALL	SETBASE
SAME2FP:PUSH    AF              ; save first character
        ...
        POP     AF              ; restore first character
	JR	L2D40		; NXT-DGT-2


; a leading fractional point has been found in a number.
...
L2CFF:  CALL    L2D1B           ; routine NUMERIC
        ...
        CALL    DEC2FP		; routine INT-TO-FP places exponent on stack
...
L2E25:  DEFB	$02		;;delete         BUGFIX
	DEFB    $E2             ;;get-mem-2      int x = 0, x-int x.
        ...
        LD      HL,MEM5        ; address MEM-5-1 leading digit counter
...
; ( or in the case of mem-5 will become zero as a result of RLD instruction )
        ...
        LD      HL,MEM4        ; address MEM-4-0 byte destination.
        LD      DE,MEM3        ; address MEM-3-0 nibble source.
...
; be printed using E-format notation
        ...
        LD      A,(MEM5)       ; fetch digit count from MEM-5-1st
...
L2F0C:  PUSH    AF              ; save A and flags
        LD      HL,MEM3        ; address MEM-3 start of digits
...
; machine stack.
        ...
        LD      BC,(MEM5)      ; set C to MEM-5-1st digit counter.
                                ; set B to MEM-5-2nd leading digit counter.
        LD      HL,MEM3        ; set HL to start of digits at MEM-3-1
        LD      A,B             ;
        CP      $09             ;
...
;   True binary operations.
        ...
        DEFW    TOPWR           ; $06 Address: $3851 - to-power
...
L33B4:  LD      DE,(STKEND)      ; Load destination from STKEND system variable.
        ...
        CALL    L33C0           ; Routine MOVE-FP puts on calculator stack
                                ; with a memory check.
L33BB:	LD      (STKEND),DE      ; Set STKEND to next free location.
...
; The whole thing saves less memory than its code
...
;;;        DEC     A               ; decrease
; This is somewhat faster. Take advantage of the saved space by removing LD DE,0
;;;	JR	NZ,L33F9
;;;	RET
;;;	NOP
;;;	NOP
...
STRINGS:CALL    L2BF1           ; routine STK-FETCH gets 2nd string params
        PUSH    DE              ; save start2 *.
...
; This routine restores DEF FN variables from the stack.
; It has been designed with minimum RAM and ROM use in mind, not maximum
; performance. Takes 52 bytes of ROM space

CNTARGS:LD	HL,(STKEND)	; STKEND
	LD	BC,5
CNTARG:	LD	A,(DE)
	CP	$0E
	INC	DE
...
RSTARGS:PUSH	DE
	CALL	CNTARGS
	SBC	HL,BC
	POP	DE		; DEF FN argument list
	PUSH	HL		; SCANNING result location
RSTARG:	LD	A,(DE)
	CP	$0E
	INC	DE
	JR	NZ,RNOARG
...
RNOARG:	CP	")"
	JR	NZ,RSTARG
	POP	DE		; SCANNING result location
...
; Reset stream retaining shadow registers for
RESET_STREAM_SAVE:
	EXX
	PUSH	BC
	PUSH	DE
