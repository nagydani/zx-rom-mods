...
PR_PRINTER:
	EX	AF,AF'
	XOR	A
	CP	(IY+PR_CC+1-ERR_NR)	; PRINTER BUFFER
	JR	NZ,PR_PR_E
	LD	HL,(PROG)
	LD	BC,$000B
	ADD	HL,BC
	...
	POP	BC
; This is needed for ZX Interface 1 compatibility
	LD	DE,5
	ADD	HL,DE
	LD	(HL),"T"	; dummy channel
	ADD	HL,DE
	LD	(HL),C
	INC	HL
	LD	(HL),B
...
PR_OUT:	BIT	1,(IY+FLAGS-ERR_NR)	; Printer in use?
	CALL	NZ,PR_PRINTER
	JR	NC,IOCTL
PR_OUT1:LD	C,$FF
	IN	C,(C)
...
PR_TOKS:BIT	2,(IY+TV_FLAG-ERR_NR)	; instruction?
	JR	NZ,PR_INST		; jump, if so
	PUSH	HL
	PUSH	BC
	LD	C,A
	LD	HL,FN_TABLE
...
PR_INST:RES	2,(IY+TV_FLAG-ERR_NR)	; avoid recursion
	PUSH	HL
	PUSH	BC
	CP	"{"		; ON_ERR_T
	JR	NZ, PR_INS1
	RES	2,(IY+FLAGS-ERR_NR)	; stay in K mode
PR_INS1:LD	HL,TK_TABLE
	LD	C,A
	CALL	INDEXER
	JR	C,TK_INST
	LD	A,C
...
LEADING:BIT	0,(IY+FLAGS-ERR_NR)	; leading space
	RET	NZ
	PUSH	AF
	LD	A," "
	RST	$30
